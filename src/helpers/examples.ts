...
export async function getRepoInfo(url: URL, examplePath?: string): Promise<RepoInfo | undefined> {
  ...
  // Support repos whose entire purpose is to be a Memberstack example, e.g.
  // https://github.com/:username/:my-cool-memberstack-example-repo-name.
  if (t === undefined) {
...
export function existsInRepo(nameOrUrl: string): Promise<boolean> {
  try {
    ...
    return isUrlOk(
      `https://api.github.com/repos/memberstack/create-memberstack-app/contents/examples/${encodeURIComponent(
        nameOrUrl
      )}`
    );
  }
...
export function downloadAndExtractExample(root: string, name: string): Promise<void> {
  ...
  return pipeline(
    got.stream('https://codeload.github.com/memberstack/create-memberstack-app/tar.gz/main'),
    tar.extract({ cwd: root, strip: 3 }, [`create-memberstack-app-main/examples/${name}`])
  );
}
